from utils import (
	...
	stack_all_channels_of_CC_and_calculate_MAD,detect,
	reweight_by_common_station,write_catalog)
import pandas as pd
import os,torch,h5py,glob,yaml,time
...
class TemplateMatching(torch.utils.data.Dataset):
	def __init__(self,args):
		...
		candidate_folders = sorted(glob.glob(os.path.join(args['root'],'*')))[9:] # comment this line
		skip = -1 # change it to test for a specific date
		if continued:
...
#				print('Skip until %d template ID for the first day.'%skip)
		...
		self.number_stations_threshold = args['number_stations_threshold_for_template']
		radius = args['radius']
		...
		self.max_number_chan_in_one_station = args['max_number_channels_in_one_station']
	def __len__(self):
		return len(self.ctlg)*len(self.candidate_folders)
	...
	def __getitem__(self,idx):
		ci,ti = idx//len(self.ctlg),idx%len(self.ctlg)
#		ci,ti = 3,6
...
#		if ti<1234:return {'flag':0} # skip some templates for test
...
for x in pipe:
	...
	ArrayStack,MAD,Mask,Lefts,N_high_cc_channels,N_high_cc_stations = stack_all_channels_of_CC_and_calculate_MAD(
		CC,mask_zero_frac,shifts,snr,reweight,args['minimum_cc'],args['device'])
	gids,tids = detect(ArrayStack,MAD,Lefts,N_high_cc_channels,N_high_cc_stations,
		args['mad_threshold'],args['number_high_cc_channels_threshold'],args['number_high_cc_stations_threshold'],
		int(args['too_close_detections_to_remain']*args['sampling_rate']),args['device'])
	if len(gids):write_catalog(gids,tids,N_high_cc_channels,N_high_cc_stations,folder,Lefts,evloc,
					grids,MAD,ArrayStack,shifts,template,continuous,mask_zero_frac,channels,CC,dists,args)
	print('Runtime: %f s'%(time.time()-t0))